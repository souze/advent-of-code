...
logic input =
    input
        ...
        |> show String.lines
        |> show (List.map parse)
        |> show (List.map unwrapResult)
        |> show (List.foldl helper (Acc [] Dict.empty))
        |> show (\a -> a.dirSizes)
        |> show findNeeded
        |> show fixit
        |> show (List.head >> unwrap)


fixit ( num, d ) =
    d
        |> Dict.toList
        |> List.filter (\( _, s ) -> s >= num)
        |> List.sortBy Tuple.second


findNeeded : Dict (List String) Int -> ( Int, Dict (List String) Int )
findNeeded d =
    ( 30000000 - (70000000 - (Dict.get [ "/" ] d |> unwrap)), d )


type alias Acc =
    { path : List String
    , dirSizes : Dict (List String) Int
    }


helper : ParseOutput -> Acc -> Acc
helper po acc =
    case po of
        Command (Cd "..") ->
            { acc | path = List.drop 1 acc.path }
        ...
        DirOrFile (File size) ->
            { acc
                | dirSizes =
                    acc.path
                        |> List.foldr
                            (\d ( newDict, path ) -> ( addSize size (d :: path) newDict, d :: path ))
                            ( acc.dirSizes, [] )
                        |> Tuple.first
            }
...
addSize : Int -> List String -> Dict (List String) Int -> Dict (List String) Int
addSize s name data =
    Dict.update
        name
        (\maybe_d ->
...
createParseOutput =
    Debug.todo "trying to create parse output"
